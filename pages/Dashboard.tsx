              ...
              courses.map(course => (
                <div key={course._id} className={`bg-white/70 backdrop-blur-2xl p-6 rounded-[32px] shadow-lg border transition-all duration-300 ${selectedCourseId === course._id ? 'border-orange-600 shadow-orange-600/10' : 'border-white/40 hover:border-orange-200'}`}>
                    ...
                    <div className="flex gap-3">
                        <button onClick={() => onNavigate(`#/course-details/${course._id}`)} className="flex-1 py-2 bg-orange-600 text-white rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-orange-700 transition-all shadow-lg shadow-orange-600/20 active:scale-95 flex items-center justify-center gap-2">
                            <BookOpen size={14} /> View Details
                        </button>
                        <button onClick={() => onNavigate(`#/edit-course/${course._id}`)} className="flex-1 py-2 bg-white border border-slate-200 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center justify-center gap-2 text-slate-600">
                            <Edit size={14} /> Edit Course
                        </button>
                        <button onClick={() => handleDelete(course._id)} className="p-2 bg-slate-50 border border-slate-100 rounded-full hover:bg-rose-50 hover:text-rose-600 transition-all flex items-center justify-center text-slate-300">
                            <Trash2 size={14} />
                        </button>
                            </div>
                </div>
