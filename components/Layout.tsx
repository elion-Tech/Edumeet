...
export const Layout: React.FC<LayoutProps> = ({ user, onLogout, onNavigate, children }) => {
  ...
  const NavItem = ({ icon: Icon, label, path, active }: any) => (
    <button 
      ...
      className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-full transition-all duration-300 group relative overflow-hidden ${
        active 
          ? 'bg-orange-500/10 text-orange-500 shadow-[0_0_20px_rgba(249,115,22,0.1)] border border-orange-500/20' 
          : 'text-slate-400 hover:bg-white/5 hover:text-slate-200 hover:translate-x-1'
      }`}
    >
      {active && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-orange-500 rounded-r-full shadow-[0_0_10px_#f97316]" />}
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f8fafc] font-sans selection:bg-orange-100 selection:text-orange-600">
      {/* Mobile Overlay */}
      {isMobileMenuOpen && (
      ...
      <nav className={`fixed md:relative inset-y-0 left-0 z-50 w-full md:w-72 bg-[#0f172a] text-white flex flex-col shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${isMobileMenuOpen ? 'translate-y-0' : '-translate-y-full md:translate-y-0 md:translate-x-0'} md:h-screen overflow-hidden`}>
        ...
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
            <div className="absolute -top-[20%] -left-[20%] w-[70%] h-[40%] bg-orange-500/10 blur-[100px] rounded-full mix-blend-screen" />
            <div className="absolute top-[40%] -right-[20%] w-[60%] h-[40%] bg-rose-500/10 blur-[100px] rounded-full mix-blend-screen" />
        </div>
        ...
        <div className="relative z-10 p-6 md:p-8 border-b border-white/5">
          ...
          {user && (
            <div className="mt-8 p-4 bg-white/5 rounded-3xl border border-white/5 flex items-center gap-4 backdrop-blur-sm">
              <div className="w-10 h-10 bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl flex items-center justify-center text-white shadow-inner">
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-[11px] font-medium text-orange-300/80 uppercase tracking-wider">{user?.role}</p>
              </div>
            </div>
          )}
        </div>
        ...
        {user && (
          <div className="relative z-10 p-6 border-t border-white/5">
            <button 
              onClick={onLogout}
              className="flex items-center gap-3 text-slate-400 hover:text-rose-400 transition-all duration-300 w-full px-4 py-3 rounded-full hover:bg-rose-500/10 font-medium text-sm group"
            >
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="h-20 mx-4 md:mx-6 mt-4 mb-2 rounded-[32px] bg-white/80 backdrop-blur-xl border border-white/60 shadow-sm flex items-center justify-between px-4 md:px-8 z-30 transition-all hover:shadow-md sticky top-4">
          ...
          {user && (
            <div className="flex items-center gap-6">
              <div className="relative">
                <button 
                  className={`relative p-3 rounded-full transition-all duration-300 hover:scale-105 active:scale-95 ${showNotifs ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/20' : 'bg-white border border-slate-200/60 text-slate-500 hover:text-orange-600 hover:border-orange-200 hover:shadow-md'}`}
                ...
                {showNotifs && (
                  <div className="absolute right-0 mt-4 w-80 bg-white/90 backdrop-blur-xl rounded-[32px] shadow-2xl border border-slate-100 overflow-hidden z-[60] animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-200">
                    ...
                    <div className="max-h-[450px] overflow-y-auto p-4 space-y-2">
                      {safeNotifications.length === 0 ? (
                        ...
                        safeNotifications.map(notif => (
                          <div key={notif._id} className={`p-4 rounded-2xl border transition-all hover:scale-[1.02] ${!notif.read ? 'bg-orange-50/80 border-orange-100 shadow-sm' : 'bg-transparent border-transparent opacity-70'}`}>
                            <div className="flex justify-between items-start mb-2">
                              <span className="font-bold text-xs text-orange-600">{notif.fromName}</span>
                            </div>
                            <p className="text-sm text-slate-600 mb-3 leading-relaxed">{notif.message}</p>
                            {!notif.read && (
                              <button 
                                onClick={() => handleMarkRead(notif._id)}
                                className="w-full py-2 bg-white border border-orange-100 text-orange-600 hover:bg-orange-50 rounded-lg text-xs font-semibold transition-colors shadow-sm"
                              >
                                Mark as read
                              </button>
                            )}
